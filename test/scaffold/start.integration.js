...
var TachacoinService = require('../../lib/services/tachacoind');
...
describe('#start', function() {
  ...
  describe('will dynamically create a node from a configuration', function() {
    ...
    it('require each tachacoincore-node service with default config', function(done) {
      var node;
      var TestNode = function(options) {
        options.services[0].should.deep.equal({
          name: 'tachacoind',
          module: TachacoinService,
          config: {
            spawn: {
              datadir: './data'
            }
    ...
    it('require each tachacoincore-node service with explicit config', function(done) {
      var node;
      var TestNode = function(options) {
        options.services[0].should.deep.equal({
          name: 'tachacoind',
