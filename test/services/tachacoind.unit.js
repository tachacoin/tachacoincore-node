...
var Transaction = tachacoincore.Transaction;
var readFileSync = sinon.stub().returns(fs.readFileSync(path.resolve(__dirname, '../data/tachacoin.conf')));
var TachacoinService = proxyquire('../../lib/services/tachacoind', {
	fs: {
		readFileSync: readFileSync
	}
});
var defaultTachacoinConf = fs.readFileSync(path.resolve(__dirname, '../data/default.tachacoin.conf'), 'utf8');

describe('Tachacoin Service', function() {
	...
	var baseConfig = {
		node: {
			network: tachacoincore.Networks.testnet
		},
		spawn: {
			datadir: 'testdir',
			exec: 'testpath'
	...
	describe('@constructor', function() {
		it('will create an instance', function() {
			var tachacoind = new TachacoinService(baseConfig);
			should.exist(tachacoind);
		});
		it('will create an instance without `new`', function() {
			var tachacoind = TachacoinService(baseConfig);
			should.exist(tachacoind);
		});
		it('will init caches', function() {
			var tachacoind = new TachacoinService(baseConfig);

			should.exist(tachacoind.utxosCache);
			should.exist(tachacoind.txidsCache);
			should.exist(tachacoind.balanceCache);
			should.exist(tachacoind.summaryCache);
			should.exist(tachacoind.transactionDetailedCache);
			should.exist(tachacoind.accountInfo);

			should.exist(tachacoind.transactionCache);
			should.exist(tachacoind.rawTransactionCache);
			should.exist(tachacoind.rawJsonTransactionCache);
			should.exist(tachacoind.blockCache);
			should.exist(tachacoind.blockJsonCache);
			should.exist(tachacoind.blockSubsidyCache);
			should.exist(tachacoind.rawBlockCache);
			should.exist(tachacoind.blockHeaderCache);
			should.exist(tachacoind.zmqKnownTransactions);
			should.exist(tachacoind.zmqKnownBlocks);
			should.exist(tachacoind.lastTip);
			should.exist(tachacoind.lastTipTimeout);
			should.equal(tachacoind.dgpInfoCache, null, 'should be null');
			should.equal(tachacoind.miningInfoCache, null, 'should be null');
			should.equal(tachacoind.stakingInfoCache, null, 'should be null');

			// limits
			should.equal(tachacoind.maxTxids, 1000);
			should.equal(tachacoind.maxTransactionHistory, 50);
			should.equal(tachacoind.maxAddressesQuery, 10000);
			should.equal(tachacoind.shutdownTimeout, 15000);

			// spawn restart setting
			should.equal(tachacoind.spawnRestartTime, 5000);
			should.equal(tachacoind.spawnStopTime, 10000);

			// try all interval
			should.equal(tachacoind.tryAllInterval, 1000);
			should.equal(tachacoind.startRetryInterval, 5000);

			// rpc limits
			should.equal(tachacoind.transactionConcurrency, 5);

			// sync progress level when zmq subscribes to events
			should.equal(tachacoind.zmqSubscribeProgress, 0.9999);
		});
		it('will init clients', function() {
			var tachacoind = new TachacoinService(baseConfig);
			tachacoind.nodes.should.deep.equal([]);
			tachacoind.nodesIndex.should.equal(0);
			tachacoind.nodes.push({ client: sinon.stub() });
			should.exist(tachacoind.client);
		});
		it('will set subscriptions', function() {
			var tachacoind = new TachacoinService(baseConfig);
			tachacoind.subscriptions.should.deep.equal({
				address: {},
				rawtransaction: [],
				hashblock: [],
				balance: {},
	...
	describe('#_initDefaults', function() {
		it('will set transaction concurrency', function() {
			var tachacoind = new TachacoinService(baseConfig);
			tachacoind._initDefaults({ transactionConcurrency: 10 });
			tachacoind.transactionConcurrency.should.equal(10);
			tachacoind._initDefaults({});
			tachacoind.transactionConcurrency.should.equal(5);
		});
	});

	describe('@dependencies', function() {
		it('will have no dependencies', function() {
			TachacoinService.dependencies.should.deep.equal([]);
		});
	});
	...
	describe('#getAPIMethods', function() {
		it('will return spec', function() {
			var tachacoind = new TachacoinService(baseConfig);
			var methods = tachacoind.getAPIMethods();
	...
	describe('#getPublishEvents', function() {
		it('will return spec', function() {
			var tachacoind = new TachacoinService(baseConfig);
			var events = tachacoind.getPublishEvents();
			should.exist(events);
			events.length.should.equal(4);

			events[0].name.should.equal('tachacoind/rawtransaction');
			events[0].scope.should.equal(tachacoind);
			events[0].subscribe.should.be.a('function');
			events[0].unsubscribe.should.be.a('function');

			events[1].name.should.equal('tachacoind/hashblock');
			events[1].scope.should.equal(tachacoind);
			events[1].subscribe.should.be.a('function');
			events[1].unsubscribe.should.be.a('function');

			events[2].name.should.equal('tachacoind/addresstxid');
			events[2].scope.should.equal(tachacoind);
			events[2].subscribe.should.be.a('function');
			events[2].unsubscribe.should.be.a('function');

			events[3].name.should.equal('tachacoind/addressbalance');
			events[3].scope.should.equal(tachacoind);
		...
		it('will call subscribe/unsubscribe with correct args', function() {
			var tachacoind = new TachacoinService(baseConfig);
			tachacoind.subscribe = sinon.stub();
			tachacoind.unsubscribe = sinon.stub();
			var events = tachacoind.getPublishEvents();

			events[0].subscribe('test');
			tachacoind.subscribe.args[0][0].should.equal('rawtransaction');
			tachacoind.subscribe.args[0][1].should.equal('test');

			events[0].unsubscribe('test');
			tachacoind.unsubscribe.args[0][0].should.equal('rawtransaction');
			tachacoind.unsubscribe.args[0][1].should.equal('test');

			events[1].subscribe('test');
			tachacoind.subscribe.args[1][0].should.equal('hashblock');
			tachacoind.subscribe.args[1][1].should.equal('test');

			events[1].unsubscribe('test');
			tachacoind.unsubscribe.args[1][0].should.equal('hashblock');
			tachacoind.unsubscribe.args[1][1].should.equal('test');
		});
	});
	...
	describe('#subscribe', function() {
		...
		it('will push to subscriptions', function() {
			var tachacoind = new TachacoinService(baseConfig);
			var emitter = {};
			tachacoind.subscribe('hashblock', emitter);
			tachacoind.subscriptions.hashblock[0].should.equal(emitter);

			var emitter2 = {};
			tachacoind.subscribe('rawtransaction', emitter2);
			tachacoind.subscriptions.rawtransaction[0].should.equal(emitter2);
		});
	});
	...
	describe('#unsubscribe', function() {
		...
		it('will remove item from subscriptions', function() {
			...
			var emitter4 = {};
			var emitter5 = {};
			tachacoind.subscribe('hashblock', emitter1);
			tachacoind.subscribe('hashblock', emitter2);
			tachacoind.subscribe('hashblock', emitter3);
			tachacoind.subscribe('hashblock', emitter4);
			tachacoind.subscribe('hashblock', emitter5);
			tachacoind.subscriptions.hashblock.length.should.equal(5);

			tachacoind.unsubscribe('hashblock', emitter3);
			tachacoind.subscriptions.hashblock.length.should.equal(4);
			tachacoind.subscriptions.hashblock[0].should.equal(emitter1);
			tachacoind.subscriptions.hashblock[1].should.equal(emitter2);
			tachacoind.subscriptions.hashblock[2].should.equal(emitter4);
			tachacoind.subscriptions.hashblock[3].should.equal(emitter5);
		});
		it('will not remove item an already unsubscribed item', function() {
			var tachacoind = new TachacoinService(baseConfig);
			var emitter1 = {};
			var emitter3 = {};
			tachacoind.subscriptions.hashblock = [emitter1];
			tachacoind.unsubscribe('hashblock', emitter3);
			tachacoind.subscriptions.hashblock.length.should.equal(1);
			tachacoind.subscriptions.hashblock[0].should.equal(emitter1);
		});
	});
	...
	describe('#subscribeAddress', function() {
		...
		it('will not an invalid address', function() {
			...
			tachacoind.subscribeAddress(emitter, ['invalidaddress']);
			should.not.exist(tachacoind.subscriptions.address['invalidaddress']);
		});
		it('will add a valid address', function() {
			var tachacoind = new TachacoinService(baseConfig);
			var emitter = new EventEmitter();
			tachacoind.subscribeAddress(emitter, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(tachacoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
		});
		it('will handle multiple address subscribers', function() {
			var tachacoind = new TachacoinService(baseConfig);
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			tachacoind.subscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			tachacoind.subscribeAddress(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(tachacoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			tachacoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
		});
		it('will not add the same emitter twice', function() {
			...
			tachacoind.subscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(tachacoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			tachacoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		});
	});
	...
	describe('#unsubscribeAddress', function() {
		...
		it('it will remove a subscription', function() {
			...
			should.exist(tachacoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			tachacoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
			tachacoind.unsubscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			tachacoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		});
		it('will unsubscribe subscriptions for an emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			tachacoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			tachacoind.unsubscribeAddress(emitter1);
			tachacoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		});
		it('will NOT unsubscribe subscription with missing address', function() {
			...
			var emitter2 = new EventEmitter();
			tachacoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			tachacoind.unsubscribeAddress(emitter1, ['qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX']);
			tachacoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
		});
		it('will NOT unsubscribe subscription with missing emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			tachacoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter2];
			tachacoind.unsubscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			tachacoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			tachacoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'][0].should.equal(emitter2);
		});
		it('will remove empty addresses', function() {
			...
			tachacoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			tachacoind.unsubscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			tachacoind.unsubscribeAddress(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.not.exist(tachacoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
		});
		it('will unsubscribe emitter for all addresses', function() {
			...
			var emitter2 = new EventEmitter();
			tachacoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			tachacoind.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			sinon.spy(tachacoind, 'unsubscribeAddressAll');
			tachacoind.unsubscribeAddress(emitter1);
			tachacoind.unsubscribeAddressAll.callCount.should.equal(1);
			tachacoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			tachacoind.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
		});
	});
	...
	describe('#unsubscribeAddressAll', function() {
		...
		it('will unsubscribe emitter for all addresses', function() {
			...
			tachacoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			tachacoind.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			tachacoind.subscriptions.address['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'] = [emitter2];
			tachacoind.subscriptions.address['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR'] = [emitter1];
			tachacoind.unsubscribeAddressAll(emitter1);
			tachacoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			tachacoind.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
			tachacoind.subscriptions.address['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'].length.should.equal(1);
			should.not.exist(tachacoind.subscriptions.address['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR']);
		});
	});
	...
	describe('#_getDefaultConfig', function() {
		it('will generate config file from defaults', function() {
			var tachacoind = new TachacoinService(baseConfig);
			var config = tachacoind._getDefaultConfig();
			config.should.equal(defaultTachacoinConf);
		});
	});
	...
	describe('#_loadSpawnConfiguration', function() {
		...
		it('will parse a tachacoin.conf file', function() {
			var TestTachacoin = proxyquire('../../lib/services/tachacoind', {
				fs: {
					readFileSync: readFileSync,
			...
			var tachacoind = new TestTachacoin(baseConfig);
			tachacoind.options.spawn.datadir = '/tmp/.tachacoin';

			sinon.spy(tachacoind, '_expandRelativeDatadir');
			sinon.spy(tachacoind, '_checkConfigIndexes');
			sinon.spy(tachacoind, '_getDefaultConf')
			sinon.spy(tachacoind, '_getNetworkConfigPath');
			sinon.spy(tachacoind, '_parseBitcoinConf');

			var node = {};
			tachacoind._loadSpawnConfiguration(node);

			tachacoind._expandRelativeDatadir.callCount.should.equal(1);
			tachacoind._checkConfigIndexes.callCount.should.equal(1);
			tachacoind._getDefaultConf.callCount.should.equal(1);
			tachacoind._getNetworkConfigPath.callCount.should.equal(1);
			tachacoind._parseBitcoinConf.callCount.should.equal(2);

			should.exist(tachacoind.spawn.config);
			tachacoind.spawn.config.should.deep.equal({
				addressindex: 1,
				checkblocks: 144,
				dbcache: 8192,
		...
		it('will expand relative datadir to absolute path', function() {
			...
			var config = {
				node: {
					network: tachacoincore.Networks.testnet,
					configPath: '/tmp/.tachacoincore/tachacoincore-node.json'
				},
				spawn: {
					datadir: './data',
					exec: 'testpath'
			...
			var tachacoind = new TestTachacoin(config);
			...
			tachacoind.options.spawn.datadir = './data';
			var node = {};
			tachacoind._loadSpawnConfiguration(node);
			...
			tachacoind.options.spawn.datadir.should.equal('/tmp/.tachacoincore/data');
		});
		it('should throw an exception if txindex isn\'t enabled in the configuration', function() {
			var TestTachacoin = proxyquire('../../lib/services/tachacoind', {
				fs: {
					readFileSync: sinon.stub().returns(fs.readFileSync(__dirname + '/../data/badtachacoin.conf')),
					existsSync: sinon.stub().returns(true),
				},
				mkdirp: {
					sync: sinon.stub()
			...
			var tachacoind = new TestTachacoin(baseConfig);
			(function() {
				tachacoind._loadSpawnConfiguration({ datadir: './test' });
			}).should.throw(tachacoincore.errors.InvalidState);
		});
	...
	describe('#_checkConfigIndexes', function() {
		...
		it('should warn the user if txindex isn\'t set to 1 in the tachacoin.conf file', function() {
			var tachacoind = new TachacoinService(baseConfig);
			var config = {
				txindex: 0,
				addressindex: 1,
			...
			var node = {};
			(function() {
				tachacoind._checkConfigIndexes(config, node);
			}).should.throw('"txindex" option');
		});
		it('should warn the user if addressindex isn\'t set to 1 in the tachacoin.conf file', function() {
			var tachacoind = new TachacoinService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 0,
		...
		it('should warn the user if spentindex isn\'t set to 1 in the tachacoin.conf file', function() {
			var tachacoind = new TachacoinService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 1,
		...
		it('should warn the user if server isn\'t set to 1 in the tachacoin.conf file', function() {
			var tachacoind = new TachacoinService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 1,
		...
		it('should warn the user if reindex is set to 1 in the tachacoin.conf file', function() {
			var tachacoind = new TachacoinService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 1,
	...
	describe('#_resetCaches', function() {
		it('will reset LRU caches', function() {
			...
			for (var i = 0; i < 10; i++) {
				...
				tachacoind.transactionDetailedCache.set(keys[i], {});
				tachacoind.utxosCache.set(keys[i], {});
				tachacoind.accountInfo.set(keys[i], {});
				tachacoind.txidsCache.set(keys[i], {});
				tachacoind.balanceCache.set(keys[i], {});
				tachacoind.summaryCache.set(keys[i], {});
				tachacoind.blockOverviewCache.set(keys[i], {});
				tachacoind.blockJsonCache.set(keys[i], {});
			}
			tachacoind._resetCaches();
			should.equal(tachacoind.transactionDetailedCache.get(keys[0]), undefined);
			should.equal(tachacoind.utxosCache.get(keys[0]), undefined);
			should.equal(tachacoind.accountInfo.get(keys[0]), undefined);
			should.equal(tachacoind.txidsCache.get(keys[0]), undefined);
			should.equal(tachacoind.balanceCache.get(keys[0]), undefined);
			should.equal(tachacoind.summaryCache.get(keys[0]), undefined);
			should.equal(tachacoind.blockOverviewCache.get(keys[0]), undefined);
			should.equal(tachacoind.blockJsonCache.get(keys[0]), undefined);
	...
	describe('#_tryAllClients', function() {
		it('will retry for each node client', function(done) {
			var tachacoind = new TachacoinService(baseConfig);
			tachacoind.tryAllInterval = 1;
			tachacoind.nodes.push({
				client: {
			...
			tachacoind._tryAllClients(function(client, next) {
				client.getInfo(next);
			}, function(err) {
				if (err) {
					return done(err);
				}
				tachacoind.nodes[0].client.getInfo.callCount.should.equal(1);
				tachacoind.nodes[1].client.getInfo.callCount.should.equal(1);
				tachacoind.nodes[2].client.getInfo.callCount.should.equal(1);
				done();
			});
		});
		it('will start using the current node index (round-robin)', function(done) {
			...
			tachacoind._tryAllClients(function(client, next) {
				...
				tachacoind.nodes[1].client.getInfo.callCount.should.equal(1);
				tachacoind.nodes[2].client.getInfo.callCount.should.equal(1);
				tachacoind.nodesIndex.should.equal(2);
				done();
			});
		});
	...
	describe('#_wrapRPCError', function() {
		it('will convert tachacoind-rpc error object into JavaScript error', function() {
			var tachacoind = new TachacoinService(baseConfig);
			var error = tachacoind._wrapRPCError({ message: 'Test error', code: -1 });
	...
	describe('#_initChain', function() {
		...
		it('will set height and genesis buffer', function(done) {
			...
			tachacoind.getRawBlock = sinon.stub().callsArgWith(1, null, genesisBuffer);
			tachacoind.nodes.push({
				client: {
			...
			tachacoind._initChain(function() {
				log.info.callCount.should.equal(1);
				tachacoind.getRawBlock.callCount.should.equal(1);
				tachacoind.getRawBlock.args[0][0].should.equal('genesishash');
				tachacoind.height.should.equal(5000);
				tachacoind.genesisBuffer.should.equal(genesisBuffer);
				done();
			});
		});
		it('it will handle error from getBestBlockHash', function(done) {
			...
			tachacoind._initChain(function(err) {
		...
		it('it will handle error from getRawBlock', function(done) {
			...
			tachacoind.getRawBlock = sinon.stub().callsArgWith(1, new Error('test'));
			tachacoind._initChain(function(err) {
				err.should.be.instanceOf(Error);
				getBestBlockHash.callCount.should.equal(1);
				getBlock.callCount.should.equal(1);
	...
	describe('#_getDefaultConf', function() {
		afterEach(function() {
			tachacoincore.Networks.disableRegtest();
			baseConfig.node.network = tachacoincore.Networks.testnet;
		});
		it('will get default rpc port for livenet', function() {
			var config = {
				node: {
					network: tachacoincore.Networks.livenet
				},
				spawn: {
					datadir: 'testdir',
					exec: 'testpath'
			...
			var tachacoind = new TachacoinService(config);
			tachacoind._getDefaultConf().rpcport.should.equal(8332);
		});
		it('will get default rpc port for testnet', function() {
			...
			var tachacoind = new TachacoinService(config);
			tachacoind._getDefaultConf().rpcport.should.equal(18332);
		});
	...
	describe('#_getNetworkConfigPath', function() {
		...
		it('will get default config path for livenet', function() {
			...
			var tachacoind = new TachacoinService(config);
			should.equal(tachacoind._getNetworkConfigPath(), undefined);
		});
		it('will get default rpc port for testnet', function() {
			var config = {
				node: {
			...
			var tachacoind = new TachacoinService(config);
			tachacoind._getNetworkConfigPath().should.equal('testnet3/tachacoin.conf');
		});
		it('will get default rpc port for regtest', function() {
			tachacoincore.Networks.enableRegtest();
			var config = {
			...
			var tachacoind = new TachacoinService(config);
			tachacoind._getNetworkConfigPath().should.equal('regtest/tachacoin.conf');
		});
	});
	...
	describe('#_getNetworkOption', function() {
		...
		it('return --testnet for testnet', function() {
			var tachacoind = new TachacoinService(baseConfig);
			tachacoind.node.network = tachacoincore.Networks.testnet;
			tachacoind._getNetworkOption().should.equal('--testnet');
		});
		it('return --regtest for testnet', function() {
			var tachacoind = new TachacoinService(baseConfig);
			tachacoind.node.network = tachacoincore.Networks.testnet;
			tachacoincore.Networks.enableRegtest();
			tachacoind._getNetworkOption().should.equal('--regtest');
		});
		it('return undefined for livenet', function() {
			var tachacoind = new TachacoinService(baseConfig);
			tachacoind.node.network = tachacoincore.Networks.livenet;
			tachacoincore.Networks.enableRegtest();
			should.equal(tachacoind._getNetworkOption(), undefined);
		});
	});
	...
	describe('#_zmqBlockHandler', function() {
		it('will emit block', function(done) {
			...
			tachacoind._rapidProtectedUpdateTip = sinon.stub();
			tachacoind.on('block', function(block) {
				block.should.equal(message);
				done();
			});
			tachacoind._zmqBlockHandler(node, message);
			tachacoind._rapidProtectedUpdateTip.callCount.should.equal(1);
		});
		it('will not emit same block twice', function(done) {
			...
			var message = new Buffer('76843f1dfe455347d897e757ff5ff51a16a0f62b5d02a659c5680392de3a2b89', 'hex');
			tachacoind._rapidProtectedUpdateTip = sinon.stub();
			sinon.spy(tachacoind, 'emit');
			tachacoind.on('block', function(block) {
				block.should.equal(message);
				tachacoind.emit.callCount.should.equal(1);
				tachacoind._rapidProtectedUpdateTip.callCount.should.equal(1);
				done();
			});
			tachacoind._zmqBlockHandler(node, message);
		...
		it('will call function to update tip', function() {
			...
			tachacoind._zmqBlockHandler(node, message);
			tachacoind._rapidProtectedUpdateTip.callCount.should.equal(1);
			tachacoind._rapidProtectedUpdateTip.args[0][0].should.equal(node);
			tachacoind._rapidProtectedUpdateTip.args[0][1].should.equal(message);
		});
		it('will emit to subscribers', function(done) {
			...
			sinon.spy(tachacoind, 'emit');
			var emitter = new EventEmitter();
			tachacoind.subscriptions.hashblock.push(emitter);
			emitter.on('tachacoind/hashblock', function(blockHash) {
				tachacoind._rapidProtectedUpdateTip.callCount.should.equal(1);
				tachacoind.emit.callCount.should.equal(1);
	...
	describe('#_rapidProtectedUpdateTip', function() {
		it('will limit tip updates with rapid calls', function(done) {
			var tachacoind = new TachacoinService(baseConfig);
			var callCount = 0;
			tachacoind._updateTip = function() {
				callCount++;
			...
			var count = 0;
			function repeat() {
				tachacoind._rapidProtectedUpdateTip(node, message);
				count++;
				if (count < 50) {
					repeat();
				}
	...
	describe('#_updateTip', function() {
		...
		it('log and emit rpc error from get block', function(done) {
			var tachacoind = new TachacoinService(baseConfig);
			tachacoind.syncPercentage = sinon.stub();
			sinon.spy(tachacoind, '_resetCaches');
			tachacoind.on('error', function(err) {
				err.code.should.equal(-1);
				err.message.should.equal('Test error');
				node.client.getBlock.callCount.should.equal(1);
				tachacoind._resetCaches.callCount.should.equal(1);
			...
			tachacoind._updateTip(node, message);
		});
		it('emit synced if percentage is 100', function(done) {
			var tachacoind = new TachacoinService(baseConfig);
			tachacoind.syncPercentage = sinon.stub().callsArgWith(0, null, 100);
			tachacoind.on('synced', function() {
				tachacoind.syncPercentage.callCount.should.equal(1);
				node.client.getBlock.callCount.should.equal(1);
				done();
			});
			var node = {
		...
		it('NOT emit synced if percentage is less than 100', function(done) {
			var tachacoind = new TachacoinService(baseConfig);
			tachacoind.syncPercentage = sinon.stub().callsArgWith(0, null, 99);
			tachacoind.on('synced', function() {
				tachacoind.syncPercentage.callCount.should.equal(0);
				node.client.getBlock.callCount.should.equal(1);
		...
		it('log and emit error from syncPercentage', function(done) {
			var tachacoind = new TachacoinService(baseConfig);
			tachacoind.syncPercentage = sinon.stub().callsArgWith(0, new Error('test'));
			tachacoind.on('error', function(err) {
				log.error.callCount.should.equal(1);
				tachacoind.syncPercentage.callCount.should.equal(1);
				node.client.getBlock.callCount.should.equal(1);
		...
		it('reset caches and set height', function(done) {
			var tachacoind = new TachacoinService(baseConfig);
			tachacoind.syncPercentage = sinon.stub();
			tachacoind._resetCaches = sinon.stub();
			tachacoind.on('tip', function(height) {
				tachacoind._resetCaches.callCount.should.equal(1);
				height.should.equal(10);
				tachacoind.height.should.equal(10);
				done();
			});
			var node = {
				client: {
	...
	describe('#_getAddressesFromTransaction', function() {
		it('will get results using tachacoincore.Transaction', function() {
			var tachacoind = new TachacoinService(baseConfig);
			var wif = 'cVTiUNavxbCzuoaQboDcgEhKYHhgXRaeQ8ZRoYkVXE9mxT1TcSrc';
			var privkey = tachacoincore.PrivateKey.fromWIF(wif);
			var inputAddress = privkey.toAddress(tachacoincore.Networks.testnet);
			var outputAddress = tachacoincore.Address('qMDYc9oXQK19dmLSCQNs5okRt8z4XeMr1X');
			var tx = tachacoincore.Transaction();
			tx.from({
				txid: '63ea7d22cb412283f4c6964a0fddebf3069f2e8a4761e86b5d0e2a39b0e84e5c',
				outputIndex: 0,
				script: tachacoincore.Script(inputAddress),
				address: inputAddress.toString(),
				satoshis: 5000000000
			});
			tx.to(outputAddress, 5000000000);
			tx.sign(privkey);
			var addresses = tachacoind._getAddressesFromTransaction(tx);
		...
		it('will handle non-standard script types', function() {
			var tachacoind = new TachacoinService(baseConfig);
			var tx = tachacoincore.Transaction();
			tx.addInput(tachacoincore.Transaction.Input({
				prevTxId: '63ea7d22cb412283f4c6964a0fddebf3069f2e8a4761e86b5d0e2a39b0e84e5c',
				script: tachacoincore.Script('OP_TRUE'),
				outputIndex: 1,
				output: {
					script: tachacoincore.Script('OP_TRUE'),
					satoshis: 5000000000
			...
			tx.addOutput(tachacoincore.Transaction.Output({
				script: tachacoincore.Script('OP_TRUE'),
				satoshis: 5000000000
			}));
			var addresses = tachacoind._getAddressesFromTransaction(tx);
		...
		it('will return unique values', function() {
			var tachacoind = new TachacoinService(baseConfig);
			var tx = tachacoincore.Transaction();
			var address = tachacoincore.Address('qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX');
			tx.addOutput(tachacoincore.Transaction.Output({
				script: tachacoincore.Script(address),
				satoshis: 5000000000
			}));
			tx.addOutput(tachacoincore.Transaction.Output({
				script: tachacoincore.Script(address),
	...
	describe('#_notifyAddressTxidSubscribers', function() {
		it('will emit event if matching addresses', function(done) {
			...
			tachacoind._getAddressesFromTransaction = sinon.stub().returns([address]);
			var emitter = new EventEmitter();
			tachacoind.subscriptions.address[address] = [emitter];
			var txid = 'a9f5d9dc39bfff65c12d8a7a8263f57cc187d4c6cc62815d65186726bbc5ecb5';
			var transaction = {};
			emitter.on('tachacoind/addresstxid', function(data) {
			...
			tachacoind._notifyAddressTxidSubscribers(txid, transaction);
		});
	...
	describe('#_zmqTransactionHandler', function() {
		it('will emit to subscribers', function(done) {
			...
			var emitter = new EventEmitter();
			tachacoind.subscriptions.rawtransaction.push(emitter);
			emitter.on('tachacoind/rawtransaction', function(hex) {
				hex.should.be.a('string');
				hex.should.equal(expectedBuffer.toString('hex'));
				done();
			});
			var node = {};
			tachacoind._zmqTransactionHandler(node, expectedBuffer);
		});
		it('will NOT emit to subscribers more than once for the same tx', function(done) {
			...
			sinon.spy(tachacoind, 'emit');
			tachacoind.subscriptions.rawtransaction.push(emitter);
			emitter.on('tachacoind/rawtransaction', function() {
				done();
			});
			var node = {};
			tachacoind._zmqTransactionHandler(node, expectedBuffer);
		...
		it('will emit "tx" event', function(done) {
			var tachacoind = new TachacoinService(baseConfig);
			var expectedBuffer = new Buffer(txhex, 'hex');
			tachacoind.on('tx', function(buffer) {
	...
	describe('#_checkSyncedAndSubscribeZmqEvents', function() {
		...
		it('log errors, update tip and subscribe to zmq events', function(done) {
			var tachacoind = new TachacoinService(baseConfig);
			tachacoind._updateTip = sinon.stub();
			tachacoind._subscribeZmqEvents = sinon.stub();
			var blockEvents = 0;
			tachacoind.on('block', function() {
				blockEvents++;
			});
			var getBestBlockHash = sinon.stub().callsArgWith(0, null, {
			...
			tachacoind._checkSyncedAndSubscribeZmqEvents(node);
			setTimeout(function() {
				...
				tachacoind._updateTip.callCount.should.equal(11);
				tachacoind._subscribeZmqEvents.callCount.should.equal(1);
				done();
			}, 200);
		});
	...
	describe('#_subscribeZmqEvents', function() {
		it('will call subscribe on zmq socket', function() {
			...
			tachacoind._subscribeZmqEvents(node);
		...
		it('will call relevant handler for rawtx topics', function(done) {
			var tachacoind = new TachacoinService(baseConfig);
			tachacoind._zmqTransactionHandler = sinon.stub();
			var node = {
			...
			node.zmqSubSocket.on('message', function() {
				tachacoind._zmqTransactionHandler.callCount.should.equal(1);
				done();
			});
		...
		it('will call relevant handler for hashblock topics', function(done) {
			var tachacoind = new TachacoinService(baseConfig);
			tachacoind._zmqBlockHandler = sinon.stub();
			var node = {
				zmqSubSocket: new EventEmitter()
			};
			node.zmqSubSocket.subscribe = sinon.stub();
			tachacoind._subscribeZmqEvents(node);
			node.zmqSubSocket.on('message', function() {
				tachacoind._zmqBlockHandler.callCount.should.equal(1);
				done();
			});
		...
		it('will ignore unknown topic types', function(done) {
			...
			tachacoind._subscribeZmqEvents(node);
			node.zmqSubSocket.on('message', function() {
				tachacoind._zmqBlockHandler.callCount.should.equal(0);
				tachacoind._zmqTransactionHandler.callCount.should.equal(0);
				done();
			});
	...
	describe('#_initZmqSubSocket', function() {
		it('will setup zmq socket', function() {
			...
			var tachacoind = new TachacoinService(baseConfig);
			var node = {};
			tachacoind._initZmqSubSocket(node, 'url');
	...
	describe('#_checkReindex', function() {
		...
		it('give error from client getblockchaininfo', function(done) {
			...
			tachacoind._checkReindex(node, function(err) {
				should.exist(err);
		...
		it('will wait until sync is 100 percent', function(done) {
			...
			tachacoind._checkReindex(node, function() {
	...
	describe('#_loadTipFromNode', function() {
		...
		it('will give rpc error from client getbestblockhash', function(done) {
			...
			tachacoind._loadTipFromNode(node, function(err) {
				err.should.be.instanceof(Error);
		...
		it('will set height and emit tip', function(done) {
			...
			tachacoind.on('tip', function(height) {
				...
				tachacoind.height.should.equal(100);
				done();
			});
			tachacoind._loadTipFromNode(node, function(err) {
				if (err) {
	...
	describe('#_stopSpawnedProcess', function() {
		...
		it('it will kill process and resume', function(done) {
			...
			var TestTachacoinService = proxyquire('../../lib/services/tachacoind', {
				fs: {
					readFile: readFile
				}
			});
			var tachacoind = new TestTachacoinService(baseConfig);
			tachacoind.spawnStopTime = 1;
			tachacoind._process = {};
			tachacoind._process.kill = sinon.stub();
			tachacoind._stopSpawnedBitcoin(function(err) {
				if (err) {
					return done(err);
				}
				tachacoind._process.kill.callCount.should.equal(1);
				log.warn.callCount.should.equal(1);
				done();
			});
		});
		it('it will attempt to kill process and resume', function(done) {
			...
			error2.code = 'ESRCH';
			tachacoind._process.kill = sinon.stub().throws(error2);
			tachacoind._stopSpawnedBitcoin(function(err) {
				if (err) {
					return done(err);
				}
	...
	describe('#_spawnChildProcess', function() {
		...
		it('will give error from spawn config', function(done) {
			var tachacoind = new TachacoinService(baseConfig);
			tachacoind._loadSpawnConfiguration = sinon.stub();
			tachacoind._loadSpawnConfiguration = sinon.stub().throws(new Error('test'));
			tachacoind._spawnChildProcess(function(err) {
				tachacoind._loadSpawnConfiguration.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				err.message.should.equal('test');
				done();
			});
		...
		it('will give error from stopSpawnedBitcoin', function() {
			var tachacoind = new TachacoinService(baseConfig);
			tachacoind._loadSpawnConfiguration = sinon.stub();
			tachacoind._stopSpawnedBitcoin = sinon.stub().callsArgWith(0, new Error('test'));
			tachacoind._spawnChildProcess(function(err) {
				tachacoind._loadSpawnConfiguration.callCount.should.equal(1);
				tachacoind._stopSpawnedBitcoin.callCount.should.equal(1);
				err.should.be.instanceOf(Error);
				err.message.should.equal('test');
			});
		});
		it('will exit spawn if shutdown', function() {
			...
			var tachacoind = new TestTachacoinService(config);
			tachacoind.spawn = {};
			tachacoind._loadSpawnConfiguration = sinon.stub();
			tachacoind._stopSpawnedBitcoin = sinon.stub().callsArgWith(0, null);
			tachacoind.node.stopping = true;
			tachacoind._spawnChildProcess(function(err) {
				err.should.be.instanceOf(Error);
		...
		it('will include network with spawn command and init zmq/rpc on node', function(done) {
			...
			tachacoind._loadSpawnConfiguration = sinon.stub();
			tachacoind.spawn = {};
			tachacoind.spawn.exec = 'testexec';
			tachacoind.spawn.configPath = 'testdir/tachacoin.conf';
			tachacoind.spawn.datadir = 'testdir';
			tachacoind.spawn.config = {};
			tachacoind.spawn.config.rpcport = 20001;
			tachacoind.spawn.config.rpcuser = 'tachacoin';
			tachacoind.spawn.config.rpcpassword = 'password';
			tachacoind.spawn.config.zmqpubrawtx = 'tcp://127.0.0.1:30001';

			tachacoind._loadTipFromNode = sinon.stub().callsArgWith(1, null);
			tachacoind._initZmqSubSocket = sinon.stub();
			tachacoind._checkSyncedAndSubscribeZmqEvents = sinon.stub();
			tachacoind._checkReindex = sinon.stub().callsArgWith(1, null);
			tachacoind._spawnChildProcess(function(err, node) {
				should.not.exist(err);
				spawn.callCount.should.equal(1);
				spawn.args[0][0].should.equal('testexec');
				spawn.args[0][1].should.deep.equal([
					'--conf=testdir/tachacoin.conf',
					'--datadir=testdir',
					'--testnet'
				]);
				...
				tachacoind._loadTipFromNode.callCount.should.equal(1);
				tachacoind._initZmqSubSocket.callCount.should.equal(1);
				should.exist(tachacoind._initZmqSubSocket.args[0][0].client);
				tachacoind._initZmqSubSocket.args[0][1].should.equal('tcp://127.0.0.1:30001');
				tachacoind._checkSyncedAndSubscribeZmqEvents.callCount.should.equal(1);
				should.exist(tachacoind._checkSyncedAndSubscribeZmqEvents.args[0][0].client);
				should.exist(node);
				should.exist(node.client);
				done();
			});
		});
		it('will respawn tachacoind spawned process', function(done) {
			var process = new EventEmitter();
			var spawn = sinon.stub().returns(process);
			var TestTachacoinService = proxyquire('../../lib/services/tachacoind', {
				fs: {
			...
			tachacoind._loadSpawnConfiguration = sinon.stub();
			tachacoind.spawn = {};
			tachacoind.spawn.exec = 'tachacoind';
			tachacoind.spawn.datadir = '/tmp/tachacoin';
			tachacoind.spawn.configPath = '/tmp/tachacoin/tachacoin.conf';
			tachacoind.spawn.config = {};
			tachacoind.spawnRestartTime = 1;
			tachacoind._loadTipFromNode = sinon.stub().callsArg(1);
			tachacoind._initZmqSubSocket = sinon.stub();
			tachacoind._checkReindex = sinon.stub().callsArg(1);
			tachacoind._checkSyncedAndSubscribeZmqEvents = sinon.stub();
			tachacoind._stopSpawnedBitcoin = sinon.stub().callsArg(0);
			sinon.spy(tachacoind, '_spawnChildProcess');
			tachacoind._spawnChildProcess(function(err) {
				if (err) {
					return done(err);
				}
				process.once('exit', function() {
					setTimeout(function() {
						tachacoind._spawnChildProcess.callCount.should.equal(2);
						done();
					}, 5);
				});
				process.emit('exit', 1);
		...
		it('will emit error during respawn', function(done) {
			...
			tachacoind._spawnChildProcess(function(err) {
				...
				tachacoind._spawnChildProcess = sinon.stub().callsArgWith(0, new Error('test'));
				tachacoind.on('error', function(err) {
					err.should.be.instanceOf(Error);
					err.message.should.equal('test');
					done();
		...
		it('will NOT respawn tachacoind spawned process if shutting down', function(done) {
			var process = new EventEmitter();
			var spawn = sinon.stub().returns(process);
			var TestTachacoinService = proxyquire('../../lib/services/tachacoind', {
				fs: {
			...
			tachacoind._spawnChildProcess(function(err) {
				...
				process.once('exit', function() {
					setTimeout(function() {
						tachacoind._spawnChildProcess.callCount.should.equal(1);
						done();
					}, 5);
				});
				process.emit('exit', 1);
		...
		it('will give error after 60 retries', function(done) {
			...
			var tachacoind = new TestTachacoinService(baseConfig);
			tachacoind.startRetryInterval = 1;
			tachacoind._loadSpawnConfiguration = sinon.stub();
			tachacoind.spawn = {};
			tachacoind.spawn.exec = 'testexec';
			tachacoind.spawn.configPath = 'testdir/tachacoin.conf';
			...
			tachacoind.spawn.config.rpcpassword = 'password';
			tachacoind.spawn.config.zmqpubrawtx = 'tcp://127.0.0.1:30001';
			tachacoind._loadTipFromNode = sinon.stub().callsArgWith(1, new Error('test'));
			tachacoind._spawnChildProcess(function(err) {
				tachacoind._loadTipFromNode.callCount.should.equal(60);
				err.should.be.instanceof(Error);
				done();
			});
		});
		it('will give error from check reindex', function(done) {
			...
			tachacoind._initZmqSubSocket = sinon.stub();
			tachacoind._checkSyncedAndSubscribeZmqEvents = sinon.stub();
			tachacoind._checkReindex = sinon.stub().callsArgWith(1, new Error('test'));
	...
	describe('#_connectProcess', function() {
		it('will give error if connecting while shutting down', function(done) {
			...
			var tachacoind = new TachacoinService(config);
			tachacoind.node.stopping = true;
			tachacoind.startRetryInterval = 100;
			tachacoind._loadTipFromNode = sinon.stub();
			tachacoind._connectProcess({}, function(err) {
				err.should.be.instanceof(Error);
				err.message.should.match(/Stopping while trying to connect to tachacoind/);
				tachacoind._loadTipFromNode.callCount.should.equal(0);
				done();
			});
		});
		it('will give error from loadTipFromNode after 60 retries', function(done) {
			...
			tachacoind.startRetryInterval = 1;
			var config = {};
			tachacoind._connectProcess(config, function(err) {
				err.should.be.instanceof(Error);
				tachacoind._loadTipFromNode.callCount.should.equal(60);
				done();
			});
		...
		it('will init zmq/rpc on node', function(done) {
			...
			tachacoind._loadTipFromNode = sinon.stub().callsArgWith(1, null);
			var config = {};
			tachacoind._connectProcess(config, function(err, node) {
				should.not.exist(err);
				tachacoind._loadTipFromNode.callCount.should.equal(1);
				tachacoind._initZmqSubSocket.callCount.should.equal(1);
				tachacoind._loadTipFromNode.callCount.should.equal(1);
	...
	describe('#start', function() {
		...
		it('will give error if "spawn" and "connect" are both not configured', function(done) {
			var tachacoind = new TachacoinService(baseConfig);
			tachacoind.options = {};
			tachacoind.start(function(err) {
				err.should.be.instanceof(Error);
				err.message.should.be.a('string').and.equal('Tachacoin configuration options "spawn" or "connect" are expected');
				done();
			});
		});
		...
		it('will give error from connectProcess', function(done) {
			var tachacoind = new TachacoinService(baseConfig);
			tachacoind._connectProcess = sinon.stub().callsArgWith(1, new Error('test'));
			tachacoind.options = {
				connect: [
					{}
				]
			...
			tachacoind.start(function(err) {
				tachacoind._connectProcess.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				err.message.should.equal('test');
				done();
			});
		...
		it('will push node from spawnChildProcess', function(done) {
			var tachacoind = new TachacoinService(baseConfig);
			var node = {};
			tachacoind._initChain = sinon.stub().callsArg(0);
			tachacoind._spawnChildProcess = sinon.stub().callsArgWith(0, null, node);
			tachacoind.options = {
				spawn: {}
			};
			tachacoind.start(function(err) {
				should.not.exist(err);
				tachacoind.nodes.length.should.equal(1);
				done();
			});
		});
		it('will push node from connectProcess', function(done) {
			...
			sinon.spy(tachacoind, '_spawnChildProcess');
			var nodes = [{}];
			tachacoind._connectProcess = sinon.stub().callsArgWith(1, null, nodes);
			tachacoind.options = {
				connect: [
					{}
				]
			...
			tachacoind.start(function(err) {
				should.not.exist(err);
				tachacoind._initChain.callCount.should.equal(1);
				tachacoind._connectProcess.callCount.should.equal(1);
				tachacoind.nodes.length.should.equal(1);
				done();
			});
	...
	describe('#isSynced', function() {
		it('will give error from syncPercentage', function(done) {
			var tachacoind = new TachacoinService(baseConfig);
			tachacoind.syncPercentage = sinon.stub().callsArgWith(0, new Error('test'));
			tachacoind.isSynced(function(err) {
				should.exist(err);
				tachacoind.syncPercentage.callCount.should.equal(1);
				err.message.should.equal('test');
				done();
		...
		it('will give "true" if percentage is 100.00', function(done) {
			var tachacoind = new TachacoinService(baseConfig);
			tachacoind.syncPercentage = sinon.stub().callsArgWith(0, null, 100.00);
			tachacoind.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
				tachacoind.syncPercentage.callCount.should.equal(1);
		...
		it('will give "true" if percentage is 99.98', function(done) {
			var tachacoind = new TachacoinService(baseConfig);
			tachacoind.syncPercentage = sinon.stub().callsArgWith(0, null, 99.98);
			tachacoind.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
		...
		it('will give "false" if percentage is 99.49', function(done) {
			var tachacoind = new TachacoinService(baseConfig);
			tachacoind.syncPercentage = sinon.stub().callsArgWith(0, null, 99.49);
			tachacoind.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
		...
		it('will give "false" if percentage is 1', function(done) {
			var tachacoind = new TachacoinService(baseConfig);
			tachacoind.syncPercentage = sinon.stub().callsArgWith(0, null, 1);
			tachacoind.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
	...
	describe('#syncPercentage', function() {
		it('will give rpc error', function(done) {
			...
			tachacoind.syncPercentage(function(err) {
				should.exist(err);
				tachacoind.nodes[0].client.getBlockchainInfo.callCount.should.equal(1);
		...
		it('will call client getInfo and give result', function(done) {
			...
			tachacoind.syncPercentage(function(err, percentage) {
				if (err) {
					return done(err);
				}
				tachacoind.nodes[0].client.getBlockchainInfo.callCount.should.equal(1);
	...
	describe('#_normalizeAddressArg', function() {
		it('will turn single address into array', function() {
			var tachacoind = new TachacoinService(baseConfig);
			var args = tachacoind._normalizeAddressArg('address');
			args.should.deep.equal(['address']);
		});
		it('will keep an array as an array', function() {
			var tachacoind = new TachacoinService(baseConfig);
			var args = tachacoind._normalizeAddressArg(['address one', 'address two']);
	...
	describe('#getAddressBalance', function() {
		it('will give rpc error', function(done) {
			var tachacoind = new TachacoinService(baseConfig);
			sinon.spy(tachacoind, '_normalizeAddressArg');
			tachacoind.nodes.push({
				client: {
			...
			var options = {};
			tachacoind.getAddressBalance(address, options, function(err) {
				tachacoind._normalizeAddressArg.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				done();
			});
		});
		it('will give balance', function(done) {
			...
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			var options = {};
			tachacoind.getAddressBalance(address, options, function(err, data) {
				if (err) {
					return done(err);
				}
				data.balance.should.equal(10000);
				data.received.should.equal(100000);
				tachacoind.getAddressBalance(address, options, function(err, data2) {
					if (err) {
						return done(err);
					}
					data2.balance.should.equal(10000);
					data2.received.should.equal(100000);
					tachacoind._normalizeAddressArg.callCount.should.equal(2);
	...
	describe('#getAddressUnspentOutputs', function() {
		it('will give rpc error', function(done) {
			...
			tachacoind.getAddressUnspentOutputs(address, options, function(err) {
				should.exist(err);
				err.should.be.instanceof(errors.RPCError);
				done();
			});
		...
		it('will give results from client getaddressutxos', function(done) {
			...
			var address = 'qMYemSxspKpzB2RbDunJRosQv5PJd3eEWD';
			tachacoind.getAddressUnspentOutputs(address, options, function(err, utxos) {
				if (err) {
					return done(err);
				}
	...
	describe('#_getBalanceFromMempool', function() {
		it('will sum satoshis', function() {
			...
			var sum = tachacoind._getBalanceFromMempool(deltas);
	...
	describe('#_getTxidsFromMempool', function() {
		it('will filter to txids', function() {
			...
			var txids = tachacoind._getTxidsFromMempool(deltas);
	...
	describe('#_getHeightRangeQuery', function() {
		it('will detect range query', function() {
			...
			var rangeQuery = tachacoind._getHeightRangeQuery(options);
		...
		it('will get range properties', function() {
			...
			var clone = {};
			var rangeQuery = tachacoind._getHeightRangeQuery(options, clone);
		...
		it('will throw error with invalid range', function() {
			...
			(function() {
				tachacoind._getHeightRangeQuery(options);
	...
	describe('#getAddressTxids', function() {
		it('will give error from _getHeightRangeQuery', function(done) {
			var tachacoind = new TachacoinService(baseConfig);
			tachacoind._getHeightRangeQuery = sinon.stub().throws(new Error('test'));
			tachacoind.getAddressTxids('address', {}, function(err) {
				err.should.be.instanceOf(Error);
				err.message.should.equal('test');
				done();
			});
		...
		it('will give rpc error from mempool query', function() {
			...
			sinon.spy(tachacoind, '_getHeightRangeQuery');
			sinon.spy(tachacoind, '_normalizeAddressArg');
			...
			var options = {};
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			tachacoind.getAddressTxids(address, options, function(err) {
				should.exist(err);
				tachacoind._getHeightRangeQuery.callCount.should.equal(1);
				tachacoind._normalizeAddressArg.callCount.should.equal(1);
				err.should.be.instanceof(errors.RPCError);
			});
		});
		...
		it('will get txid results', function(done) {
			...
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			tachacoind.getAddressTxids(address, options, function(err, txids) {
				if (err) {
					return done(err);
				}
				tachacoind._getHeightRangeQuery.callCount.should.equal(1);
		...
		it('will get txid results from cache', function(done) {
			...
			tachacoind.getAddressTxids(address, options, function(err, txids) {
				...
				tachacoind.getAddressTxids(address, options, function(err, txids) {
					...
					tachacoind._getHeightRangeQuery.callCount.should.equal(2);
					tachacoind._normalizeAddressArg.callCount.should.equal(2);
					getAddressTxids.callCount.should.equal(1);
					txids.should.deep.equal(expectedTxids);
					done();
		...
		it('will get txid results WITHOUT cache if rangeQuery and exclude mempool', function(done) {
			...
			tachacoind.getAddressTxids(address, options, function(err, txids) {
				...
				tachacoind.getAddressTxids(address, options, function(err, txids) {
					...
					tachacoind._getHeightRangeQuery.callCount.should.equal(4);
					tachacoind._normalizeAddressArg.callCount.should.equal(2);
		...
		it('will get txid results from cache and live mempool', function(done) {
			...
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			tachacoind.getAddressTxids(address, { queryMempool: false }, function(err, txids) {
				if (err) {
					return done(err);
				}
				getAddressTxids.callCount.should.equal(1);
				...
				tachacoind.getAddressTxids(address, { queryMempool: true }, function(err, txids) {
					if (err) {
						return done(err);
					}
					getAddressTxids.callCount.should.equal(1);
					...
					tachacoind.getAddressTxids(address, { queryMempoolOnly: true }, function(err, txids) {
						if (err) {
							return done(err);
						}
						getAddressTxids.callCount.should.equal(1);
						...
						tachacoind._getHeightRangeQuery.callCount.should.equal(3);
						tachacoind._normalizeAddressArg.callCount.should.equal(3);
						done();
					});
				});
			});
	...
	describe('#_getConfirmationDetail', function() {
		...
		it('should get 0 confirmation', function() {
			...
			tx.height = -1;
			var tachacoind = new TachacoinService(baseConfig);
			tachacoind.height = 10;
			var confirmations = tachacoind._getConfirmationsDetail(tx);
	...
	describe('#_getAddressDetailsForInput', function() {
		it('will return if missing an address', function() {
			var tachacoind = new TachacoinService(baseConfig);
			var result = {};
			tachacoind._getAddressDetailsForInput({}, 0, result, []);
		...
		it('will only add address if it matches', function() {
			var tachacoind = new TachacoinService(baseConfig);
			var result = {};
			tachacoind._getAddressDetailsForInput({
				address: 'address1'
			}, 0, result, ['address2']);
			should.not.exist(result.addresses);
			should.not.exist(result.satoshis);
	...
	describe('#_getAddressDetailsForOutput', function() {
		it('will return if missing an address', function() {
			var tachacoind = new TachacoinService(baseConfig);
			var result = {};
			tachacoind._getAddressDetailsForOutput({}, 0, result, []);
			should.not.exist(result.addresses);
			should.not.exist(result.satoshis);
		});
		it('will only add address if it matches', function() {
			var tachacoind = new TachacoinService(baseConfig);
			var result = {};
			tachacoind._getAddressDetailsForOutput({
				address: 'address1'
			}, 0, result, ['address2']);
			should.not.exist(result.addresses);
			should.not.exist(result.satoshis);
	...
	describe('#_getAddressDetailsForTransaction', function() {
		it('will calculate details for the transaction', function(done) {
			...
			sinon.spy(tachacoind, '_getAddressDetailsForInput');
			sinon.spy(tachacoind, '_getAddressDetailsForOutput');

			var addresses = ['qey8KJ8xkyLUUqFZZwqngXX9U2q9ef6Tw4'];
			var details = tachacoind._getAddressDetailsForTransaction(tx, addresses);
			...
			tachacoind._getAddressDetailsForInput.callCount.should.equal(1);
			tachacoind._getAddressDetailsForOutput.callCount.should.equal(5);
			done();
		});
	});
	...
	describe('#_getAddressDetailedTransaction', function() {
		it('will get detailed transaction info', function(done) {
			...
			var tachacoind = new TachacoinService(baseConfig);
			sinon.spy(tachacoind, '_getConfirmationsDetail');
			tachacoind.getDetailedTransaction = sinon.stub().callsArgWith(1, null, tx);
			tachacoind.height = 300;
			var addresses = {};
			tachacoind._getAddressDetailsForTransaction = sinon.stub().returns({
				addresses: addresses,
				satoshis: 1000,
			});
			tachacoind._getAddressDetailedTransaction(txid, {}, function(err, details) {
				if (err) {
					return done(err);
				}
				details.addresses.should.equal(addresses);
				details.satoshis.should.equal(1000);
				tachacoind.getDetailedTransaction.callCount.should.equal(1);
				tachacoind._getAddressDetailsForTransaction.callCount.should.equal(1);
				tachacoind._getConfirmationsDetail.callCount.should.equal(1);
		...
		it('give error from _getDetailedTransaction', function(done) {
			...
			var tachacoind = new TachacoinService(baseConfig);
			tachacoind.getDetailedTransaction = sinon.stub().callsArgWith(1, new Error('test'));
			tachacoind._getAddressDetailedTransaction(txid, {}, function(err) {
				err.should.be.instanceof(Error);
				tachacoind.getDetailedTransaction.callCount.should.equal(1);
				done();
			});
	...
	describe('#_getAddressStrings', function() {
		it('will get address strings from tachacoincore addresses', function() {
			var addresses = [
				tachacoincore.Address('qU12Fa5RHM535kSDvywxPjCmbL7gwkQJZ6'),
				tachacoincore.Address('qcSLSxN1sngCWSrKFZ6UC7ri4hhVSdq9SU'),
			];
			var tachacoind = new TachacoinService(baseConfig);
			var strings = tachacoind._getAddressStrings(addresses);
		...
		it('will give error with unknown', function() {
			...
			var tachacoind = new TachacoinService(baseConfig);
			(function() {
				tachacoind._getAddressStrings(addresses);
	...
	describe('#getAddressHistory', function() {
		...
		it('will give an error if length of addresses is too long', function(done) {
			...
			var tachacoind = new TachacoinService(baseConfig);
			sinon.spy(tachacoind, '_normalizeAddressArg');
			tachacoind.maxAddressesQuery = 100;
			tachacoind.getAddressHistory(addresses, {}, function(err) {
				should.exist(err);
				tachacoind._normalizeAddressArg.callCount.should.equal(1);
		...
		it('will give error with "from" and "to" range that exceeds max size', function(done) {
			var tachacoind = new TachacoinService(baseConfig);
			sinon.spy(tachacoind, '_normalizeAddressArg');
			sinon.spy(tachacoind, '_getAddressStrings');
			tachacoind.getAddressHistory(address, { from: 0, to: 51 }, function(err) {
				should.exist(err);
				tachacoind._normalizeAddressArg.callCount.should.equal(1);
				tachacoind._getAddressStrings.callCount.should.equal(1);
		...
		it('will give error with "from" and "to" order is reversed', function(done) {
			...
			sinon.spy(tachacoind, '_normalizeAddressArg');
			sinon.spy(tachacoind, '_getAddressStrings');
			tachacoind.getAddressTxids = sinon.stub().callsArgWith(2, null, []);
			tachacoind.getAddressHistory(address, { from: 51, to: 0 }, function(err) {
				should.exist(err);
				tachacoind._normalizeAddressArg.callCount.should.equal(1);
				tachacoind._getAddressStrings.callCount.should.equal(1);
				err.message.match(/^\"from/);
		...
		it('will give error from _getAddressDetailedTransaction', function(done) {
			...
			sinon.spy(tachacoind, '_normalizeAddressArg');
			sinon.spy(tachacoind, '_getAddressStrings');
			tachacoind.getAddressTxids = sinon.stub().callsArgWith(2, null, ['txid']);
			tachacoind._getAddressDetailedTransaction = sinon.stub().callsArgWith(2, new Error('test'));
			tachacoind.getAddressHistory(address, {}, function(err) {
				should.exist(err);
				tachacoind._normalizeAddressArg.callCount.should.equal(1);
				tachacoind._getAddressStrings.callCount.should.equal(1);
				tachacoind.getAddressTxids.callCount.should.equal(1);
				tachacoind._getAddressDetailedTransaction.callCount.should.equal(1);
				err.message.should.equal('test');
				done();
			});
		});
		it('give error from getAddressTxids', function(done) {
			...
			sinon.spy(tachacoind, '_normalizeAddressArg');
			sinon.spy(tachacoind, '_getAddressStrings');
			sinon.spy(tachacoind, 'getAddressTxids');
			tachacoind.getAddressTxids = sinon.stub().callsArgWith(2, new Error('test'));
			tachacoind.getAddressHistory('address', {}, function(err) {
				should.exist(err);
				tachacoind._normalizeAddressArg.callCount.should.equal(1);
				tachacoind._getAddressStrings.callCount.should.equal(1);
		...
		it('will paginate', function(done) {
			...
			sinon.spy(tachacoind, '_normalizeAddressArg');
			sinon.spy(tachacoind, '_getAddressStrings');
			sinon.spy(tachacoind, '_paginateTxids');
			tachacoind._getAddressDetailedTransaction = function(txid, options, callback) {
				callback(null, txid);
			};
			sinon.spy(tachacoind, '_getAddressDetailedTransaction');
			var txids = ['one', 'two', 'three', 'four'];
			tachacoind.getAddressTxids = sinon.stub().callsArgWith(2, null, txids);
			tachacoind.getAddressHistory('address', { from: 1, to: 3 }, function(err, data) {
				if (err) {
					return done(err);
				}
				tachacoind._normalizeAddressArg.callCount.should.equal(1);
				tachacoind._getAddressStrings.callCount.should.equal(1);
				tachacoind.getAddressTxids.callCount.should.equal(1);
				tachacoind._getAddressDetailedTransaction.callCount.should.equal(2);
				tachacoind._paginateTxids.callCount.should.equal(1);
	...
	describe('#getAddressSummary', function() {
		...
		it('will handle error from getAddressTxids', function(done) {
			...
			tachacoind.getAddressTxids = sinon.stub().callsArgWith(2, new Error('test'));
			tachacoind.getAddressBalance = sinon.stub().callsArgWith(2, null, {});
			var address = '';
			var options = {};
			tachacoind.getAddressSummary(address, options, function(err) {
				should.exist(err);
				err.should.be.instanceof(Error);
				err.message.should.equal('test');
				done();
		...
		it('will handle error from getAddressBalance', function(done) {
			...
			tachacoind.getAddressTxids = sinon.stub().callsArgWith(2, null, {});
			tachacoind.getAddressBalance = sinon.stub().callsArgWith(2, new Error('test'), {});
			var address = '';
			var options = {};
			tachacoind.getAddressSummary(address, options, function(err) {
				should.exist(err);
		...
		it('should set all properties', function(done) {
			...
			tachacoind.getAddressTxids = sinon.stub().callsArgWith(2, null, [txid1, txid2, txid3]);
			tachacoind.getAddressBalance = sinon.stub().callsArgWith(2, null, {
				received: 30 * 1e8,
				balance: 20 * 1e8
			});
			...
			var options = {};
			tachacoind.getAddressSummary(address, options, function(err, summary) {
				tachacoind._paginateTxids.callCount.should.equal(1);
				tachacoind._normalizeAddressArg.callCount.should.equal(1);
				tachacoind.getAddressTxids.callCount.should.equal(1);
				tachacoind.getAddressBalance.callCount.should.equal(1);
				tachacoind._paginateTxids.args[0][1].should.equal(0);
				tachacoind._paginateTxids.args[0][2].should.equal(1000);
		...
		it('will get from cache with noTxList', function(done) {
			...
			tachacoind.getAddressSummary(address, options, function(err, summary) {
				...
				tachacoind.getAddressSummary(address, options, function(err, summary) {
					checkSummary(summary);
					tachacoind._paginateTxids.callCount.should.equal(0);
					tachacoind._normalizeAddressArg.callCount.should.equal(2);
					tachacoind.getAddressTxids.callCount.should.equal(1);
					tachacoind.getAddressBalance.callCount.should.equal(1);
					done();
		...
		it('will skip querying the mempool with queryMempool set to false', function(done) {
			...
			tachacoind.getAddressSummary(address, options, function() {
				getAddressMempool.callCount.should.equal(0);
				tachacoind._paginateTxids.callCount.should.equal(1);
				tachacoind._normalizeAddressArg.callCount.should.equal(1);
				tachacoind.getAddressTxids.callCount.should.equal(1);
		...
		it('will give error from _paginateTxids', function(done) {
			...
			tachacoind._paginateTxids = sinon.stub().throws(new Error('test'));
			var address = 'qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR';
			var options = {
				queryMempool: false
			};
	...
	describe('#getRawBlock', function() {
		...
		it('will give rpc error from client getblockhash', function(done) {
			...
			tachacoind.getRawBlock(10, function(err) {
				should.exist(err);
				err.should.be.instanceof(errors.RPCError);
				done();
			});
		...
		it('will give rcp error from client getblock', function(done) {
			...
			sinon.spy(tachacoind, '_tryAllClients');
			tachacoind.nodes.push({
				client: {
					getBlock: sinon.stub().callsArgWith(2, { code: -1, message: 'Test error' })
				}
			});
			tachacoind.getRawBlock(blockhash, function(err) {
				should.exist(err);
				tachacoind._tryAllClients.callCount.should.equal(1);
				err.should.be.instanceof(errors.RPCError);
				done();
			});
		});
		it('will try all nodes for getblock', function(done) {
			...
			tachacoind.getRawBlock(blockhash, function(err, buffer) {
				if (err) {
					return done(err);
				}
				buffer.should.be.instanceof(Buffer);
		...
		it('will get block by height', function(done) {
			...
			tachacoind.getRawBlock(0, function(err, buffer) {
				if (err) {
					return done(err);
				}
				buffer.should.be.instanceof(Buffer);
	...
	describe('#getBlock', function() {
		...
		it('will give an rpc error from client getblock', function(done) {
			...
			tachacoind.getBlock(0, function(err) {
				tachacoind._tryAllClients.callCount.should.equal(2);
				getBlock.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				done();
		...
		it('will getblock as tachacoincore object from height', function(done) {
			var tachacoind = new TachacoinService(baseConfig);
			sinon.spy(tachacoind, '_tryAllClients');
			var getBlock = sinon.stub().callsArgWith(2, null, {
				result: blockhex
			...
			tachacoind.getBlock(0, function(err, block) {
				should.not.exist(err);
				tachacoind._tryAllClients.callCount.should.equal(2);
				getBlock.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
				...
				block.should.be.instanceof(tachacoincore.Block);
				done();
			});
		});
		it('will getblock as tachacoincore object', function(done) {
			var tachacoind = new TachacoinService(baseConfig);
			sinon.spy(tachacoind, '_tryAllClients');
			var getBlock = sinon.stub().callsArgWith(2, null, {
				result: blockhex
			...
			tachacoind.getBlock('00000000050a6d07f583beba2d803296eb1e9d4980c4a20f206c584e89a4f02b', function(err, block) {
				should.not.exist(err);
				tachacoind._tryAllClients.callCount.should.equal(1);
				getBlock.callCount.should.equal(1);
		...
		it('will get block from cache', function(done) {
			...
			tachacoind.getBlock(hash, function(err, block) {
				should.not.exist(err);
				tachacoind._tryAllClients.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(0);
				getBlock.callCount.should.equal(1);
		...
		it('will get block from cache with height (but not height)', function(done) {
			...
			tachacoind.getBlock(0, function(err, block) {
				...
				tachacoind.getBlock(0, function(err, block) {
					should.not.exist(err);
					tachacoind._tryAllClients.callCount.should.equal(3);
	...
	describe('#getBlockHashesByTimestamp', function() {
		it('should give an rpc error', function(done) {
			...
			tachacoind.getBlockHashesByTimestamp(1517410000, 1517843954, function(err, hashes) {
				should.exist(err);
	...
	describe('#getBlockHeader', function() {
		...
		it('will give error from getBlockHash', function() {
			var tachacoind = new TachacoinService(baseConfig);
			sinon.spy(tachacoind, '_maybeGetBlockHash');
			var getBlockHash = sinon.stub().callsArgWith(1, { code: -1, message: 'Test error' });
			tachacoind.nodes.push({
				client: {
					getBlockHash: getBlockHash
			...
			tachacoind.getBlockHeader(10, function(err) {
				tachacoind._maybeGetBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
			});
		});
		it('it will give rpc error from client getblockheader', function() {
			...
			tachacoind.getBlockHeader(blockhash, function(err) {
				tachacoind._maybeGetBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
			});
		});
		it('it will give rpc error from client getblockhash', function() {
			...
			tachacoind.getBlockHeader(0, function(err) {
				tachacoind._maybeGetBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
			});
		});
		it('will give result from client getblockheader (from height)', function() {
			...
			tachacoind.getBlockHeader(81419, function(err, blockHeader) {
				should.not.exist(err);
				tachacoind._maybeGetBlockHash.callCount.should.equal(1);
		...
		it('will give result from client getblockheader (from hash)', function() {
			...
			tachacoind.getBlockHeader(blockhash, function(err, blockHeader) {
				should.not.exist(err);
				tachacoind._maybeGetBlockHash.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(0);
				blockHeader.should.deep.equal(result);
	...
	describe('#_maybeGetBlockHash', function() {
		it('will not get block hash with an address', function(done) {
			...
			tachacoind._maybeGetBlockHash('qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX', function(err, hash) {
				if (err) {
					return done(err);
				}
				tachacoind._tryAllClients.callCount.should.equal(0);
				getBlockHash.callCount.should.equal(0);
		...
		it('will not get block hash with non zero-nine numeric string', function(done) {
			...
			tachacoind._maybeGetBlockHash('109a', function(err, hash) {
				if (err) {
					return done(err);
				}
				getBlockHash.callCount.should.equal(0);
		...
		it('will get the block hash if argument is a number', function(done) {
			...
			tachacoind._maybeGetBlockHash(10, function(err, hash) {
				if (err) {
					return done(err);
				}
	...
	describe('#getBlockOverview', function() {
		...
		it('will handle error from maybeGetBlockHash', function(done) {
			var tachacoind = new TachacoinService(baseConfig);
			sinon.spy(tachacoind, '_maybeGetBlockHash');
			tachacoind._maybeGetBlockHash = sinon.stub().callsArgWith(1, new Error('test'));
			tachacoind.getBlockOverview(blockhash, function(err) {
				err.should.be.instanceOf(Error);
				tachacoind._maybeGetBlockHash.callCount.should.equal(1);
				done();
			});
		...
		it('will give expected result', function(done) {
			...
			tachacoind.getBlockOverview(blockhash, function(err, blockOverview) {
				if (err) {
					return done(err);
				}
				...
				tachacoind.getBlockOverview(blockhash, function(err, blockOverview) {
					checkBlock(blockOverview);
					tachacoind._maybeGetBlockHash.callCount.should.equal(2);
					getBlock.callCount.should.equal(1);
					done();
				});
			});
	...
	describe('#estimateFee', function() {
		it('will give rpc error', function(done) {
			...
			tachacoind.estimateFee(1, function(err) {
				should.exist(err);
		...
		it('will call client estimateFee and give result', function(done) {
			...
			tachacoind.estimateFee(1, function(err, feesPerKb) {
				if (err) {
					return done(err);
				}
				estimateFee.callCount.should.equal(1);
	...
	describe('#sendTransaction', function(done) {
		var tx = tachacoincore.Transaction(txhex);
		it('will give rpc error', function() {
			...
			tachacoind.sendTransaction(txhex, function(err) {
				should.exist(err);
		...
		it('will send to client and get hash', function() {
			...
			tachacoind.sendTransaction(txhex, function(err, hash) {
				if (err) {
					return done(err);
				}
				sendRawTransaction.callCount.should.equal(1);
		...
		it('will send to client with absurd fees and get hash', function() {
			...
			tachacoind.sendTransaction(txhex, { allowAbsurdFees: true }, function(err, hash) {
				if (err) {
					return done(err);
				}
				sendRawTransaction.callCount.should.equal(1);
		...
		it('missing callback will throw error', function() {
			...
			var transaction = tachacoincore.Transaction();
			(function() {
				tachacoind.sendTransaction(transaction);
			}).should.throw(Error);
			sendRawTransaction.callCount.should.equal(1);
		});
	});
	...
	describe('#getRawTransaction', function() {
		it('will give rpc error', function(done) {
			...
			tachacoind.getRawTransaction('txid', function(err) {
				should.exist(err);
				tachacoind._tryAllClients.callCount.should.equal(1);
		...
		it('will try all nodes', function(done) {
			...
			tachacoind.getRawTransaction('txid', function(err, tx) {
				if (err) {
					return done(err);
				}
				should.exist(tx);
	...
	describe('#getTransaction', function() {
		it('will give rpc error', function(done) {
			...
			tachacoind.getTransaction('txid', function(err) {
				should.exist(err);
				tachacoind._tryAllClients.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
				done();
		...
		it('will try all nodes', function(done) {
			...
			tachacoind.getTransaction('txid', function(err, tx) {
				if (err) {
					return done(err);
				}
				should.exist(tx);
				...
				getRawTransaction.callCount.should.equal(1);
				tachacoind._tryAllClients.callCount.should.equal(1);
				tx.should.be.an.instanceof(tachacoincore.Transaction);
				done();
			});
		});
		it('will get from cache', function(done) {
	...
	describe('#getDetailedTransaction', function() {
		...
		it('will give rpc error', function(done) {
			...
			tachacoind.getDetailedTransaction(txid, function(err) {
				should.exist(err);
				err.should.be.instanceof(errors.RPCError);
				tachacoind._tryAllClients.callCount.should.equal(1);
				done();
		...
		it('should give a transaction with all properties', function(done) {
			...
			tachacoind.getDetailedTransaction(txid, function(err, tx) {
				if (err) {
					return done(err);
				}
				checkTx(tx);
	...
	describe('#getBestBlockHash', function() {
		it('will give rpc error', function(done) {
			...
			tachacoind.getBestBlockHash(function(err) {
				should.exist(err);
				getBestBlockHash.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
				done();
		...
		it('will call client getInfo and give result', function(done) {
			...
			tachacoind.getBestBlockHash(function(err, hash) {
				if (err) {
					return done(err);
				}
				getBestBlockHash.callCount.should.equal(1);
	...
	describe('#getSpentInfo', function() {
		it('will give rpc error', function(done) {
			...
			tachacoind.getSpentInfo({}, function(err) {
				should.exist(err);
		...
		it('will empty object when not found', function(done) {
			...
			tachacoind.getSpentInfo({}, function(err, info) {
				should.not.exist(err);
				getSpentInfo.callCount.should.equal(1);
	...
	describe('#getInfo', function() {
		it('will give rpc error', function(done) {
			...
			tachacoind.getInfo(function(err) {
				should.exist(err);
				err.should.be.an.instanceof(errors.RPCError);
				done();
			});
		...
		it('will call client getInfo and give result', function(done) {
			var tachacoind = new TachacoinService(baseConfig);
			tachacoind.node.getNetworkName = sinon.stub().returns('testnet');
			...
			tachacoind.getSubsidy = sinon.stub().callsArgWith(1, null, 400000000);
			tachacoind.nodes.push({
				client: {
					getInfo: getInfo,
				},
			...
			tachacoind.getInfo(function(err, info) {
				if (err) {
					return done(err);
				}

				tachacoind.getSubsidy.callCount.should.equal(1);
				tachacoind.client.getInfo.callCount.should.equal(1);
	...
	describe('#generateBlock', function() {
		it('will give rpc error', function(done) {
			...
			tachacoind.generateBlock(10, function(err) {
				should.exist(err);
		...
		it('will call client generate and give result', function(done) {
			...
			tachacoind.generateBlock(10, function(err, hashes) {
				if (err) {
					return done(err);
				}
	...
	describe('#stop', function() {
		it('will callback if spawn is not set', function(done) {
			var tachacoind = new TachacoinService(baseConfig);
			tachacoind.stop(done);
		});
		it('will exit spawned process', function(done) {
			var tachacoind = new TachacoinService(baseConfig);
			tachacoind.spawn = {};
			tachacoind.spawn.process = new EventEmitter();
			tachacoind.spawn.process.kill = sinon.stub();
			tachacoind.stop(done);
			tachacoind.spawn.process.kill.callCount.should.equal(1);
			tachacoind.spawn.process.kill.args[0][0].should.equal('SIGINT');
			tachacoind.spawn.process.emit('exit', 0);
		});
		it('will give error with non-zero exit status code', function(done) {
			...
			tachacoind.spawn.process.kill.callCount.should.equal(1);
			tachacoind.spawn.process.kill.args[0][0].should.equal('SIGINT');
			tachacoind.spawn.process.emit('exit', 1);
		});
		it('will stop after timeout', function(done) {
			var tachacoind = new TachacoinService(baseConfig);
			tachacoind.shutdownTimeout = 300;
			tachacoind.spawn = {};
			tachacoind.spawn.process = new EventEmitter();
			tachacoind.spawn.process.kill = sinon.stub();
			tachacoind.stop(function(err) {
	...
	describe('#getAddressesMempoolBalance', function() {
		it('will give rpc error', function(done) {
			...
			tachacoind.getAddressesMempoolBalance([], {}, function(err, result) {
				should.exist(err);
				tachacoind._normalizeAddressArg.callCount.should.equal(1);
				getAddressMempool.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will calc mempool balance', function(done) {
			...
			var tachacoind = new TachacoinService(baseConfig);
			sinon.spy(tachacoind, '_normalizeAddressArg');
			sinon.spy(tachacoind, '_getBalanceFromMempool');
			var getAddressMempool = sinon.stub().callsArgWith(1, null, {
				result: deltas
			});
			tachacoind.nodes.push({
			...
			tachacoind.getAddressesMempoolBalance([], {}, function(err, result) {
				should.not.exist(err);
				tachacoind._getBalanceFromMempool.callCount.should.equal(1);
				tachacoind._normalizeAddressArg.callCount.should.equal(1);
				getAddressMempool.callCount.should.equal(1);
	...
	describe('#getJsonRawTransaction', function() {
		it('will give rpc error', function(done) {
			...
			tachacoind.getJsonRawTransaction('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae', function(err) {
				should.exist(err);
				tachacoind._tryAllClients.callCount.should.equal(1);
				getRawTransaction.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will give transaction', function(done) {
			...
			tachacoind.getJsonRawTransaction('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae', function(err, response) {
				should.not.exist(err);
				tachacoind._tryAllClients.callCount.should.equal(1);
				getRawTransaction.callCount.should.equal(1);
	...
	describe('#getSubsidy', function() {
		it('will give rpc error', function(done) {
			...
			tachacoind.getSubsidy(0, function(err) {
				should.exist(err);
		...
		it('will give subsidy', function(done) {
			...
			tachacoind.getSubsidy(0, function(err, result) {
				should.not.exist(err);
				getSubsidy.callCount.should.equal(1);
	...
	describe('#getJsonBlock', function() {
		it('will give rpc error from getBlockHash', function(done) {
			...
			tachacoind.getJsonBlock(0, function(err) {
				should.exist(err);
				tachacoind._tryAllClients.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will give rpc error from getBlock', function(done) {
			...
			tachacoind.getJsonBlock('2f46b26835e9d768edd2afd393bdaf16cb141a95a663dc1c303e086a8af05d23', function(err) {
				should.exist(err);
				tachacoind._tryAllClients.callCount.should.equal(1);
				getBlock.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will give jsonBlock using block height', function(done) {
			...
			tachacoind.getJsonBlock(80000, function(err, result) {
				should.not.exist(err);
				tachacoind._tryAllClients.callCount.should.equal(2);
				tachacoind._maybeGetBlockHash.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
		...
		it('will give jsonBlock using blockhash', function(done) {
			...
			tachacoind.getJsonBlock('2f46b26835e9d768edd2afd393bdaf16cb141a95a663dc1c303e086a8af05d23', function(err, result) {
				should.not.exist(err);
				tachacoind._tryAllClients.callCount.should.equal(1);
				tachacoind._maybeGetBlockHash.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(0);
	...
	describe('#subscribeBalance', function() {
		...
		it('will not add an invalid address', function() {
			var tachacoind = new TachacoinService(baseConfig);
			var emitter = new EventEmitter();
			tachacoind.subscribeBalance(emitter, ['invalidaddress']);
			should.not.exist(tachacoind.subscriptions.balance['invalidaddress']);
			log.info.callCount.should.equal(1);
		});
		it('will add a valid address', function() {
			var tachacoind = new TachacoinService(baseConfig);
			var emitter = new EventEmitter();
			tachacoind.subscribeBalance(emitter, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(tachacoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			log.info.callCount.should.equal(1);
		...
		it('will handle multiple address subscribers', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			tachacoind.subscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			tachacoind.subscribeBalance(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(tachacoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			tachacoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
		...
		it('will not add the same emitter twice', function() {
			...
			tachacoind.subscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(tachacoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			tachacoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			log.info.callCount.should.equal(2);
		});
	});
	...
	describe('#unsubscribeBalance', function() {
		...
		it('it will remove a subscription', function() {
			...
			should.exist(tachacoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			tachacoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
			tachacoind.unsubscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			tachacoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		...
		it('will unsubscribe subscriptions for an emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			tachacoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			tachacoind.unsubscribeBalance(emitter1);
			tachacoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			log.info.callCount.should.equal(1);
		});
		it('will NOT unsubscribe subscription with missing address', function() {
			...
			var emitter2 = new EventEmitter();
			tachacoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			tachacoind.unsubscribeBalance(emitter1, ['qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX']);
			tachacoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
			log.info.callCount.should.equal(1);
		});
		it('will NOT unsubscribe subscription with missing emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			tachacoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter2];
			tachacoind.unsubscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			tachacoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			tachacoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'][0].should.equal(emitter2);
			log.info.callCount.should.equal(1);
		});
		it('will remove empty addresses', function() {
			var tachacoind = new TachacoinService(baseConfig);
			...
			tachacoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			tachacoind.unsubscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			tachacoind.unsubscribeBalance(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.not.exist(tachacoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			log.info.callCount.should.equal(2);
		});
		it('will unsubscribe emitter for all addresses', function() {
			var tachacoind = new TachacoinService(baseConfig);
			...
			var emitter2 = new EventEmitter();
			tachacoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			tachacoind.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			sinon.spy(tachacoind, 'unsubscribeBalanceAll');
			tachacoind.unsubscribeBalance(emitter1);
			tachacoind.unsubscribeBalanceAll.callCount.should.equal(1);
			tachacoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			tachacoind.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
			log.info.callCount.should.equal(1);
		});
	});
	...
	describe('#unsubscribeBalanceAll', function() {
		...
		it('will unsubscribe emitter for all addresses', function() {
			...
			tachacoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			tachacoind.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			tachacoind.subscriptions.balance['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'] = [emitter2];
			tachacoind.subscriptions.balance['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR'] = [emitter1];
			tachacoind.unsubscribeBalanceAll(emitter1);
			tachacoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			tachacoind.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
			tachacoind.subscriptions.balance['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'].length.should.equal(1);
			should.not.exist(tachacoind.subscriptions.balance['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR']);
			log.info.callCount.should.equal(1);
		});
	});
	...
	describe('#_notifyBalanceSubscribers', function() {
		...
		it('will notify balance subscribers', function(done) {
			...
			var tachacoind = new TachacoinService(baseConfig);
			sinon.spy(tachacoind, '_notifyBalanceSubscriber');
			tachacoind._getAddressesFromTransaction = sinon.stub().returns(addresses);
			tachacoind.getAddressSummary = sinon.stub().callsArgWith(2, null, {
			...
			emitter.on('tachad/addressbalance', function(data) {
				...
				tachacoind._notifyBalanceSubscriber.callCount.should.equal(1);
				tachacoind._getAddressesFromTransaction.callCount.should.equal(1);
				emitter.emit.callCount.should.equal(1);
				done();
			});
			...
			tachacoind._notifyBalanceSubscribers('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae', transaction);
	...
	describe('#_notifyBalanceSubscriber', function() {
		...
		it('will notify balance subscriber', function(done) {
			...
			emitter.on('tachad/addressbalance', function(data) {
				...
				data.txid.should.equal('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae');
				emitter.emit.callCount.should.equal(1);
				tachacoind.getAddressSummary.callCount.should.equal(1);
				done();
			});
			...
			tachacoind._notifyBalanceSubscriber('qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z', '8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae');
	...
	describe('#listUnspent', function() {
		it('will give rpc error', function(done) {
			...
			tachacoind.listUnspent(0, 65000, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'], function(err) {
				should.exist(err);
		...
		it('will give array of unspent transaction outputs', function(done) {
			...
			tachacoind.listUnspent(0, 65000, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'], function(err, result) {
				should.not.exist(err);
				listUnspent.callCount.should.equal(1);
	...
	describe('#getNewAddress', function() {
		it('will give rpc error', function(done) {
			...
			tachacoind.getNewAddress(function(err, result) {
				should.exist(err);
	...
	describe('#callContract', function() {
		it('will give rpc error', function(done) {
			...
			tachacoind.callContract('f6177bc9812eeb531907621af6641a41133dea9e', 'd7bb99ba', {}, function(err, result) {
				should.exist(err);
	...
	describe('#getAccountInfo', function() {
		it('will give rpc error', function(done) {
			...
			tachacoind.getAccountInfo('f6177bc9812eeb531907621af6641a41133dea9e', function(err, result) {
				should.exist(err);
	...
	describe('#getTransactionReceipt', function() {
		it('will give rpc error', function(done) {
			...
			tachacoind.getTransactionReceipt('51b34cbcaf6fe3b687bf3d954f3baaeb377f5d0a3a8cdd29d899c049f6954a49', function(err, result) {
				should.exist(err);
				tachacoind._tryAllClients.callCount.should.equal(1);
	...
	describe('#getDgpInfo', function() {
		it('will give rpc error', function(done) {
			...
			tachacoind.getDgpInfo(function(err, result) {
				should.exist(err);
				tachacoind._tryAllClients.callCount.should.equal(1);
	...
	describe('#getMiningInfo', function() {
		it('will give rpc error', function(done) {
			...
			tachacoind.getMiningInfo(function(err, result) {
				should.exist(err);
				tachacoind._tryAllClients.callCount.should.equal(1);
	...
	describe('#getStakingInfo', function() {
		it('will give rpc error', function(done) {
			...
			tachacoind.getStakingInfo(function(err, result) {
				should.exist(err);
				tachacoind._tryAllClients.callCount.should.equal(1);
