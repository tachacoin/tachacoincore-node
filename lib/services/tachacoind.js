...
var TachacoinRPC = require('tachacoind-rpc');
...
/**
 * Provides a friendly event driven API to tachacoind in Node.js. Manages starting and
 * stopping tachacoind as a child process for application support, as well as connecting
 * to multiple tachacoind processes for server infrastructure. Results are cached in an
 * LRU cache for improved performance and methods added for common queries.
 *
 * @param {Object} options
 * @param {Node} options.node - A reference to the node
 */
function Tachacoin(options) {
  if (!(this instanceof Tachacoin)) {
    return new Tachacoin(options);
  }
...
util.inherits(Tachacoin, Service);

Tachacoin.dependencies = [];

Tachacoin.DEFAULT_MAX_TXIDS = 1000;
Tachacoin.DEFAULT_MAX_HISTORY = 50;
Tachacoin.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
Tachacoin.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
Tachacoin.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
Tachacoin.DEFAULT_SPAWN_RESTART_TIME = 5000;
Tachacoin.DEFAULT_SPAWN_STOP_TIME = 10000;
Tachacoin.DEFAULT_TRY_ALL_INTERVAL = 1000;
Tachacoin.DEFAULT_REINDEX_INTERVAL = 10000;
Tachacoin.DEFAULT_START_RETRY_INTERVAL = 5000;
Tachacoin.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
Tachacoin.DEFAULT_TRANSACTION_CONCURRENCY = 5;
Tachacoin.DEFAULT_CONFIG_SETTINGS = {
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
  addressindex: 1,
...
Tachacoin.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || Tachacoin.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || Tachacoin.DEFAULT_MAX_HISTORY;
  this.maxAddressesQuery = options.maxAddressesQuery || Tachacoin.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || Tachacoin.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || Tachacoin.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || Tachacoin.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || Tachacoin.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || Tachacoin.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || Tachacoin.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || Tachacoin.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

Tachacoin.prototype._initCaches = function() {
...
Tachacoin.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
Tachacoin.prototype.getAPIMethods = function() {
  var methods = [
 ...
 */
Tachacoin.prototype.getPublishEvents = function() {
  return [
    {
...
Tachacoin.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'tachacoind/' + name, 'total:', this.subscriptions[name].length);
};

Tachacoin.prototype.unsubscribe = function(name, emitter) {
  var index = this.subscriptions[name].indexOf(emitter);
  if (index > -1) {
    this.subscriptions[name].splice(index, 1);
  }
  log.info(emitter.remoteAddress, 'unsubscribe:', 'tachacoind/' + name, 'total:', this.subscriptions[name].length);
};


Tachacoin.prototype.subscribeBalance = function(emitter, addresses) {
    var self = this;
    ...
    log.info(emitter.remoteAddress, 'subscribe:', 'tachad/addressbalance', 'total:', _.size(this.subscriptions.balance));
};

Tachacoin.prototype.unsubscribeBalance = function(emitter, addresses) {
    ...
    log.info(emitter.remoteAddress, 'unsubscribe:', 'tachad/addressbalance', 'total:', _.size(this.subscriptions.balance));
};

Tachacoin.prototype.unsubscribeBalanceAll = function(emitter) {
...
Tachacoin.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
  ...
  log.info(emitter.remoteAddress, 'subscribe:', 'tachacoind/addresstxid', 'total:', _.size(this.subscriptions.address));
};

Tachacoin.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'tachacoind/addresstxid', 'total:', _.size(this.subscriptions.address));
};
 ...
 */
Tachacoin.prototype.unsubscribeAddressAll = function(emitter) {
...
Tachacoin.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = Tachacoin.DEFAULT_CONFIG_SETTINGS;
...
Tachacoin.prototype._parseBitcoinConf = function(configPath) {
  var options = {};
...
Tachacoin.prototype._expandRelativeDatadir = function() {
...
Tachacoin.prototype._loadSpawnConfiguration = function(node) {
  /* jshint maxstatements: 25 */

  $.checkArgument(this.options.spawn, 'Please specify "spawn" in tachacoind config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in tachacoind config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in tachacoind config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './tachacoin.conf');

  log.info('Using tachacoin config file:', configPath);
...
Tachacoin.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    spawnConfig.txindex && spawnConfig.txindex === 1,
    '"txindex" option is required in order to use transaction query features of tachacoincore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of tachacoincore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of tachacoincore-node. ' +
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to tachacoind from bitcore. ' +
      'Please add "server=1" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubrawtx,
    '"zmqpubrawtx" option is required to get event updates from tachacoind. ' +
      'Please add "zmqpubrawtx=tcp://127.0.0.1:<port>" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubhashblock,
    '"zmqpubhashblock" option is required to get event updates from tachacoind. ' +
  ...
  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that tachacoind is undergoing a reindex. ' +
             ...
             'of tachacoincore-node services will start.');
    node._reindex = true;
  }
};

Tachacoin.prototype._resetCaches = function() {
...
Tachacoin.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
Tachacoin.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

Tachacoin.prototype._initChain = function(callback) {
  var self = this;
    ...
    self.client.getBlock(response.result, function(err, response) {
      ...
      self.client.getBlockHash(0, function(err, response) {
        ...
        self.getRawBlock(blockhash, function(err, blockBuffer) {
          ...
          self.emit('ready');
          log.info('Tachacoin Daemon Ready');
          callback();
        });
      });
...
Tachacoin.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 8332
  };
...
Tachacoin.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === bitcore.Networks.testnet) {
    networkPath = 'testnet3/tachacoin.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/tachacoin.conf';
    }
  }
  return networkPath;
};

Tachacoin.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === bitcore.Networks.testnet) {
...
Tachacoin.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('tachacoind/hashblock', message.toString('hex'));
    }
  }

};

Tachacoin.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
Tachacoin.prototype._updateTip = function(node, message) {
  var self = this;
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('Tachacoin Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
Tachacoin.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
Tachacoin.prototype._notifyBalanceSubscribers = function(txid, transaction) {
...
Tachacoin.prototype._notifyBalanceSubscriber = function (address, txid) {
    var self = this;
    self.getAddressSummary(address, {noTxList: false}, function(err, data) {
        ...
        if (emitters) {
            for(var j = 0; j < emitters.length; j++) {
                emitters[j].emit('tachad/addressbalance', {
                    address: address,
                    txid: txid,
...
Tachacoin.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  var addresses = this._getAddressesFromTransaction(transaction);
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('tachacoind/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
Tachacoin.prototype._zmqTransactionHandler = function(node, message) {
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('tachacoind/rawtransaction', message.toString('hex'));
    }
...
Tachacoin.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || Tachacoin.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

Tachacoin.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
Tachacoin.prototype._initZmqSubSocket = function(node, zmqUrl) {
...
Tachacoin.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  if (node._reindex) {
    interval = setInterval(function() {
      node.client.getBlockchainInfo(function(err, response) {
        ...
        log.info('Tachacoin Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
    ...
    }, node._reindexWait || Tachacoin.DEFAULT_REINDEX_INTERVAL);
  } else {
    callback();
  }
};

Tachacoin.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
Tachacoin.prototype._stopSpawnedBitcoin = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/tachacoind.pid';
...
Tachacoin.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedBitcoin(function(err) {
    ...
    log.info('Starting tachacoin process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('Tachacoin process unexpectedly exited with code:', code);
        log.warn('Restarting tachacoin child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('Tachacoin process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn tachacoind.'));
      }
...
Tachacoin.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to tachacoind.'));
    }
 ...
 */
Tachacoin.prototype.start = function(callback) {
  var self = this;
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('Tachacoin configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {Function} callback
 */
Tachacoin.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
Tachacoin.prototype.syncPercentage = function(callback) {
  var self = this;
...
Tachacoin.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {String|Address|Array} addressArg - An address string, tachacoincore address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
Tachacoin.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Tachacoin.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
...
Tachacoin.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
Tachacoin.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
Tachacoin.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    if (options.end > options.start) {
      throw new TypeError('"end" is expected to be less than or equal to "start"');
    }
    if (clone) {
      // reverse start and end as the order in tachacoincore is most recent to less recent
 ...
 * @param {Function} callback
 */
Tachacoin.prototype.getAddressTxids = function(addressArg, options, callback) {
  /* jshint maxstatements: 20 */
  var self = this;
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
...
Tachacoin.prototype._getConfirmationsDetail = function(transaction) {
...
Tachacoin.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
Tachacoin.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
Tachacoin.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Object} txid - A tachacoincoin transaction id
 * @param {Function} callback
 */
Tachacoin.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
Tachacoin.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
...
Tachacoin.prototype._paginateTxids = function(fullTxids, fromArg, toArg) {
  var txids;
 ...
 * @param {Function} callback
 */
Tachacoin.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
Tachacoin.prototype.getAddressesMempoolBalance = function(addressArg, options, callback) {
    var self = this;
    var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
Tachacoin.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
...
Tachacoin.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Tachacoin.prototype.getRawBlock = function(blockArg, callback) {
 ...
 * @param {Function} callback
 */
Tachacoin.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
...
Tachacoin.prototype.getJsonBlock = function(blockArg, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
Tachacoin.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
 ...
 * @param {Function} callback
 */
Tachacoin.prototype.getBlockHashesByTimestamp = function(high, low, options, callback) {
 ...
 * @param {Function} callback
 */
Tachacoin.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Tachacoin.prototype.estimateFee = function(blocks, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Tachacoin.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Tachacoin.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Tachacoin.prototype.getJsonRawTransaction = function(txid, callback) {
  var self = this;
...
/**
 * Will get a transaction as a Tachacoincore Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
 * @param {Boolean} queryMempool - Include the mempool
 * @param {Function} callback
 */
Tachacoin.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Tachacoin.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Tachacoin.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Tachacoin.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Tachacoin.prototype.getInfo = function(callback) {
...
Tachacoin.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Tachacoin.prototype.stop = function(callback) {
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('tachacoind spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('tachacoind process did not exit'));
      }
 ...
 * @param {Array} addresses - A array of tacha addresses to filter
 * @param {Function} callback
 */
Tachacoin.prototype.listUnspent = function(minConf, maxConf, addresses, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
Tachacoin.prototype.getNewAddress = function(callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
Tachacoin.prototype.callContract = function(address, hash, options, callback) {
    var self = this,
 ...
 * @param {Function} callback
 */
Tachacoin.prototype.getAccountInfo = function(address, callback) {
 ...
 * @param {Function} callback
 */
Tachacoin.prototype.getSubsidy = function(height, callback) {
 ...
 * @param {Function} callback
 */
Tachacoin.prototype.getTransactionReceipt = function(trxHash, callback) {
 ...
 * @param {Function} callback
 */
Tachacoin.prototype.getDgpInfo = function(callback) {
 ...
 * @return {*}
 */
Tachacoin.prototype.getMiningInfo = function(callback) {
    var self = this,
 ...
 * @return {*}
 */
Tachacoin.prototype.getStakingInfo = function(callback) {
    var self = this,
