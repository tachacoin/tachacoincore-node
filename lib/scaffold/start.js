...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for tachacoind is necessary for this upgrade with the "reindex=1" bitcoin.conf option. \n' +
                 'There are changes necessary in both bitcoin.conf and tachacoincore-node.json. \n\n' +
 ...
 * @param {Object} options.config - The parsed tachacoincore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './tachacoincore-node.json');
  ...
  node.on('ready', function() {
    log.info('Tachacoincore Node ready');
  });
...
function loadModule(req, service) {
  try {
    // first try in the built-in tachacoincore-node services directory
    ...
    if (servicePackage.tachacoincoreNode) {
      serviceModule = service.name + '/' + servicePackage.tachacoincoreNode;
    }
