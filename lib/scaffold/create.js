...
var BASE_PACKAGE = {
  ...
  dependencies: {
    ...
    'tachacoincore-lib': 'https://github.com/tachacoin/tachacoincore-lib.git#master',
    'tachacoincore-node': 'https://github.com/tachacoin/tachacoincore-node.git#master'
  }
};
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/tachacoincore-node.json', configJSON);
      if (!isGlobal) {
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the tachacoincore-node directory and configuration
